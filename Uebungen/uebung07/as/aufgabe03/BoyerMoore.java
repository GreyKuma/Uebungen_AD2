...
public class BoyerMoore {
    ...
    public static int bmMatch(String text, int startIndex, int[] last, String pattern) {

        // TODO Implement here...
        int[] lastPosArray = last;
        int pLen = pattern.length();
        int i = startIndex + pLen - 1;
        int j = pLen - 1;
        do{
            if(text.charAt(i) == pattern.charAt(j)){
                ...
                int l = lastPosArray[text.charAt(i)];
                i = i + pLen - Math.min(j, 1 + l);
                j = pLen-1;
            }
    ...
    static int[] buildLastFunction(String pattern) {
        int charSetSize = 128;
        int[] LastPosArray = new int[charSetSize];
        for (int i = 0; i < charSetSize; i++) {
            LastPosArray[i] = pattern.lastIndexOf(i);
        }
        return LastPosArray;
    }
